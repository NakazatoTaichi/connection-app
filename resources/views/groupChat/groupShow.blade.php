...
@section('main')
<div class="header bg-primary bg-gradient" style="z-index: 10;">
    <div class="sender p-4 row" style="font-size: 1.5rem;">
        <div class="col-2">
            <a style="text-decoration:none; color:white;" href="{{ route('group.index') }}">
        ...
        <div class="col-8 text-center">
            <b style="color:white;">{{ $group->group_name }}</b>
        </div>
    </div>
</div>
<div class="container">
                                ...
                                <div class="message-container d-flex justify-content-start text-start">
                                    <div class="align-self-start">
                                        @if ($message->user->icon)
                                            <img src="{{ asset('storage/icons/' . $message->user->icon )}}" alt="icon" class="img-fluid rounded-circle" style="width: 50px; height: 50px; margin-right: 15px;">
                                        @else
                                            <div class="icon-placeholder rounded-circle" style="width: 50px; height: 50px; background-color: #CCCCCC; margin-right: 5px;"></div>
                                        @endif
                                    </div>
                                    <div class="group-member-message">{{ $message->message }}</div>
    ...
    <script>
        ...
        channel.bind('my-event', function(data) {
            ...
            messageDiv.className = data.user_id === {{ auth()->user()->id }} ? 'sent-message' : 'group-member-message';
